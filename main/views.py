...
from .models import Experience,ExperienceCategory,ExperienceImages,ExperienceIncluded,ExperienceFormsQ,ExperienceFormsA,MemoriesModel,UserDetails,ContactModel,ReviewsModel
...
def ExperiencesSearch(request):
    if request.method == 'POST':
        ...
        if ExperiencesList == 'alrdy' and Experiences == 'alrdy1':
            return redirect(f'/Experiences')
        elif ExperiencesList == 'alrdy':
            return redirect(f'/Experiences/{Experiences}')
        else:
...
@permission_required('is_superuser',login_url='AdminLogin')
def Admin(request):
...
def AddExperiences(request):
    ...
    if request.method=="POST": 
        ...
        dt=Experience.objects.create(EC_id=Categoryid,Category=dt.Category,Name=Name,Image=Image,SmallDescription=SmallDescription,Description=Description,Address=Address,Price=Price,Days=Days,Date=Date,EndDate=EndDate,View=View)
        dt.save()
...
def ExperiencesEdit(request,id):
    ...
    if request.method=="POST": 
        ...
        gh.EndDate=request.POST.get('EndDate')
...
def AddReviews(request):
    return render(request,'admin/addreviews.html')
